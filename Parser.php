    ...
    public static function getPage($arParams = [])
    {
        if ($arParams) {
            if (!empty($arParams["url"])) {
                ...
                $iTimeout = !empty($arParams["timeout"]) ? $arParams["timeout"] : 3000;
                $iConnectTimeout = !empty($arParams["connecttimeout"]) ? $arParams["connecttimeout"] : 3000;
                ...
                curl_setopt($rCh, CURLOPT_IPRESOLVE, CURL_IPRESOLVE_V4 );
